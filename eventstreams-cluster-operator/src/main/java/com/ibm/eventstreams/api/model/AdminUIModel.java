...
public class AdminUIModel extends AbstractModel {
    ...
    public AdminUIModel(EventStreams instance,
        ...
        Optional<String> host = userInterfaceSpec.map(AdminUISpec::getHost).filter(string -> !string.isEmpty());
        route = createRoute(getRouteName(), host, getDefaultResourceName(), UI_SERVICE_PORT, tlsConfig, new HashMap<>());
    }
    ...
    private Container getUIContainer(EventStreams instance) {
        ...
        envVarDefaults.add(new EnvVarBuilder().withName("GEOREPLICATION_ENABLED").withValue(Boolean.toString(GeoReplicatorModel.canBeSourceOrTargetForGeoReplication(instance))).build());
    ...
    private NetworkPolicy createNetworkPolicy() {
        ...
        // unrestricted ingress on 3000
        ingressRules.add(createIngressRule(UI_SERVICE_PORT, Collections.emptyList()));
